...
import { getMcpCopy } from "@/i18n/mcp-copy";
import {
...
export default function McpPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      showToast(`${copy.addFailed}: ${e}`, "error");
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleTest = async (server: McpServer) => {
    ...
    try {
      ...
      if (result.ok) {
        showToast(`${server.name}: ${result.tools.length} ${copy.tools}`, "success");
      } else {
  ...
  const handleAuthenticate = async (server: McpServer) => {
    ...
    try {
      ...
      showToast(`${server.name}: ${copy.oauthComplete}`, "success");
    } catch (e) {
      showToast(`${copy.oauthError}: ${e}`, "error");
    } finally {
    ...
    onDelete: useCallback(
      async (serverName: string) => {
        try {
          ...
          showToast(`${copy.deleted}: "${truncateText(serverName, 30)}"`, "success");
          setTestResults((prev) => {
            const next = { ...prev };
      ...
      [copy.deleted, copy.error, loadServers, showToast],
    ),
  });
  ...
  const handleInstallSubmit = () => {
    ...
    if (missing.length > 0) {
      showToast(`${missing[0].prompt} ${copy.required}`, "error");
      return;
    }
  ...
  // Put the localized creation action in the page header.
  useLayoutEffect(() => {
    setEnd(
      <Button
        className="uppercase"
      ...
      <DeleteConfirmDialog
        ...
        description={
          serverDelete.pendingId
            ? `“${truncateText(serverDelete.pendingId, 40)}” — ${copy.removeDescription}`
          ...
          >
            ...
            <header className="p-5 pb-3 border-b border-border">
              <h2
                ...
                className="font-mondwest text-display text-base tracking-wider"
              >
                {copy.installTitle} {installEntry.name}
              </h2>
            </header>
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          >
            ...
            {copy.serverList} ({servers.length})
          </H2>
        </div>
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    ...
                    {envCount > 0 && (
                      <span>
                        {envCount} {copy.environmentCount}
                      </span>
                    )}
                  </div>
                  {result && (
              ...
              <CardContent className="flex items-start gap-4 py-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <Badge tone="outline">{copy.authentication}: {entry.auth_type}</Badge>
                  ...
                  {entry.bootstrap.length > 0 && (
                    <details className="mt-1 text-xs text-muted-foreground">
                      <summary className="cursor-pointer select-none">
                        {copy.bootstrapCommands} ({entry.bootstrap.length})
                      </summary>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {entry.installed ? (
                    ...
                    >
                      {isInstalling ? copy.installing : copy.installTitle}
                    </Button>
                  )}
                </div>
              </CardContent>
