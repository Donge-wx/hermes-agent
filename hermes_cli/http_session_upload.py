...
import asyncio  # noqa: ANYIO_OK - bridges the existing asyncio TUI dispatcher
import logging
import os
import threading
import uuid
...
def _positive_int(
    raw: str | int | None, *, default: int, minimum: int, maximum: int
) -> int:
    try:
...
def _non_negative_int(value: JsonValue, field: str) -> int:
...
def register_http_session_upload(
    ...
    *,
    ...
    def tui_server_for_scope(scope: tuple[str, Path]):
        ...
        except Exception as exc:  # noqa: BROAD_EXCEPT_OK - dynamic module boundary
            raise HTTPException(
                status_code=503, detail=f"TUI gateway unavailable: {exc}"
            ) from exc
        try:
            tui_scope = tui_server._employee_tenant_scope()
        except Exception as exc:  # noqa: BROAD_EXCEPT_OK - TUI guard boundary
            raise HTTPException(
    ...
    def gateway_session(tui_server: Any, session_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(status_code=403, detail="session is outside employee home") from exc
        return session
    ...
    async def begin_session_attachment_upload(request: Request) -> dict[str, Any]:
        ...
        except Exception as exc:  # noqa: BROAD_EXCEPT_OK - request JSON boundary
    ...
    async def append_session_attachment_upload_chunk(request: Request) -> dict[str, Any]:
        ...
        def mark_transport_disconnect() -> None:
            """Retain same-id retry state, but make a new upload self-heal it."""

            try:
                tui_server._mark_desktop_file_attach_transport_disconnected(
                    session, upload_id
                )
            except Exception as exc:  # noqa: BROAD_EXCEPT_OK - recovery hint boundary
                logger.warning(
                    "Could not mark disconnected attachment upload %s: %s",
                    upload_id,
                    exc,
                    exc_info=True,
                )
        ...
        try:
            ...
            # A disconnected body used to bubble out as an ASGI 500 and leave
            # its reservation live until the 30-minute stale reaper.  Preserve
            # this upload for the desktop's same-id retry, while allowing a
            # later new begin request to reclaim it immediately if needed.
            mark_transport_disconnect()
            raise HTTPException(
                status_code=499, detail="upload client disconnected"
            ) from exc
