...
class ManagedUploadGuardError(ValueError):
    """Raised when an upload cannot be represented by the restart guard."""
...
def set_managed_upload_restart_guard(upload_id: str, *, active: bool) -> None:
    ...
    if not normalized_upload_id:
        raise ManagedUploadGuardError(
            "upload_id is required for the managed upload restart guard"
        )
