...
export const zh: Translations = {
  ...
  boot: {
    ready: 'My King 桌面版已就绪',
    desktopBootFailedWithMessage: message => `桌面启动失败：${message}`,
    steps: {
      connectingGateway: '正在连接桌面网关',
      loadingSettings: '正在加载 My King 设置',
      loadingSessions: '正在加载最近会话',
      retryingRemoteBackend: '正在重新连接远程 My King 后端…',
      startingDesktopConnection: '正在启动桌面连接',
      startingHermesDesktop: '正在启动 My King 桌面版…'
    },
    errors: {
      backgroundExited: 'My King 后台进程已退出。',
      backgroundExitedDuringStartup: 'My King 后台进程在启动期间退出。',
      backendStopped: '后端已停止',
      desktopBootFailed: '桌面启动失败',
      employeeEnrollmentRequired: '请先使用公司员工账号登录并连接。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 My King 后端早于当前桌面构建，可能无法正常工作。请更新以保持一致。',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 My King Desktop。',
    ...
    native: {
      ...
      rejectAction: '拒绝',
      inputTitle: '需要输入',
      inputBody: 'My King 正在等待你的回应。',
      turnDoneTitle: 'My King 已完成',
      turnDoneBody: '',
      turnErrorTitle: '本轮失败',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 My King 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      providers: '提供方',
    ...
    plugins: {
      title: '桌面插件',
      blurb:
        '加载到此应用中的界面扩展——随构建捆绑，或放入 desktop-plugins 文件夹（包括 My King 编写的插件）。禁用会即时卸载插件并在重启后保持。',
      count: n => `已安装 ${n} 个`,
      openFolder: '打开插件文件夹',
      rescan: '重新扫描',
      reveal: '在文件管理器中显示',
      ...
      agent: {
        title: '智能体插件',
        blurb:
          '你安装到 My King 后端的插件——工具、技能、MCP 服务器、钩子和斜杠命令。「便携」插件是 Agent Plugins 标准包（技能 + MCP 组合，也可在其他智能体中使用）。开关在新会话中生效。',
        appliesTo: '应用于：',
        empty: '尚未安装智能体插件。',
        ...
        updateBackendToManage: '请更新 My King 后端以便在桌面端管理此插件。',
      ...
      installModal: {
        ...
        agentLabel: '智能体插件',
        desktopLabel: '桌面 UI',
        agentTargetLocal: profile => `安装到 ${profile} 后端（~/.myking/plugins/）`,
    ...
    notifications: {
      ...
      focusedHint: '完成提醒仅在 My King 处于后台时触发。',
      kinds: {
        approval: {
          label: '需要批准',
        ...
        input: {
          label: '需要输入',
          description: 'My King 提出了问题，或需要密码或密钥。'
        },
        turnDone: {
          label: '回复就绪',
          description: 'My King 在后台时完成了一轮对话。'
        },
        turnError: {
          label: '本轮失败',
          description: '后台回合错误。'
        ...
        plugin: {
          label: '插件通知',
          description: 'My King 在后台时，桌面插件发送了通知。'
        }
      },
      test: '发送测试通知',
      testTitle: 'My King',
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 My King 跟随系统设置。',
      ...
      backdropTitle: '聊天背景',
      backdropDesc: '对话后方低调呈现的 My King 玻璃光晕。',
      introSplashTitle: '开场标识',
      introSplashDesc: '空白对话中显示的字标和提示语。',
      reactionsTitle: '消息回应',
      reactionsDesc: 'iMessage 风格的表情回应 — 你可以给消息添加回应，My King 也能回应你的消息。',
      ...
      themeNoMatches: query => `没有已安装主题匹配「${query}」。`,
      themeNames: { nous: '经典', 'liquid-glass': '液态玻璃' },
      themeDescriptions: {
        nous: '沉静的中性色调与 My King 蓝色强调色。',
        'liquid-glass': '珍珠、银色与苹果蓝构成的分层光学玻璃。'
      },
      pet: {
        title: '宠物',
        intro:
          '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 My King 的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
        restartHint: '宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 My King，然后回到此处。',
        scaleTitle: '大小',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: '当 My King 需要本地时间上下文时使用。留空则使用系统时区。',
      agent: {
        imageInputMode: '控制图片附件如何发送给模型。',
        maxTurns: 'My King 停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: 'My King 单次文件读取可读取的最大字符数。',
      approvals: {
        mode: 'My King 如何处理需要显式审批的命令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'My King 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'My King 后端比当前桌面安装包更新。后端更新绝不会替换品牌界面；需要新的界面功能时，请安装下一版通过验证的 My King 桌面安装包。',
      bundleOutOfSyncAction: '打开 My King 更新',
      updates: '更新',
      checkNow: '立即检查',
      checking: '检查中…',
      seeWhatsNew: '查看新增内容',
      ...
      automaticUpdatesDesc:
        'My King 会独立更新后端；品牌界面与现有布局保持不变，只有通过验证的 My King 桌面安装包才会更新前端。',
      ...
      uninstall: {
        ...
        checking: '正在检查已安装内容…',
        confirmTitle: '确认卸载',
        confirmDescription: consequence => `这将移除${consequence}，且无法撤销。`,
        appPath: path => `应用：${path}`,
        failedStart: '无法启动卸载程序。',
        uninstalling: '正在卸载…',
        confirmAction: '确认卸载',
        cancel: '取消',
        title: '卸载 My King',
        description: '请选择移除范围。应用会关闭以完成操作；之后可随时重新安装。',
        options: {
          gui: {
            title: '仅卸载桌面应用',
            description: '移除桌面应用，保留 My King 智能体、配置和聊天记录。',
            consequence: '桌面应用及其本地应用数据'
          },
          lite: {
            title: '卸载应用和智能体，保留我的数据',
            description: '移除应用与智能体，保留配置、聊天记录和密钥，便于以后重新安装。',
            consequence: '桌面应用和 My King 智能体；配置、聊天记录和密钥会保留'
          },
          full: {
            title: '全部卸载',
            description: '移除应用、智能体及所有 My King 用户数据，包括聊天、计划任务、密钥和日志。',
            consequence: '桌面应用、My King 智能体、配置、聊天记录、密钥和日志'
          }
        }
      }
    },
    config: {
      ...
      attachmentSizeDesc:
        '桌面端为预览和图片附件加载本地文件的大小上限（MB）。默认为 16。远程非图片附件使用单独的 256 MB 上限。设置过大会将整个文件读入内存，可能导致应用卡死或崩\u2060溃。',
      attachmentSizeUnit: 'MB',
      attachmentSizeLabel: '预览 / 图片加载大小上限（MB）'
    },
    quickEntry: {
      enabledTitle: '快速输入',
      enabledDesc: '用全局快捷键在任何地方唤出一个小输入框，无需打开 My King 即可发送提示。',
    ...
    connections: {
      title: '已注册网关',
      intro: '管理本机以及通过远程、SSH 或 My King Cloud 连接可访问的每个 My King 网关。',
      stagedNote:
      ...
      updateSkippedCloud: '由 My King Cloud 托管',
      kindLocal: '本地',
      kindRemote: '远程网关',
      kindCloud: 'My King Cloud',
      kindSsh: 'SSH',
      kindLocalDesc: '由本应用管理的 My King 运行时。',
      kindRemoteDesc: '可通过 HTTP(S) 访问的 My King 网关——局域网、Tailscale 或互联网。',
      kindCloudDesc: '通过你的 My King Cloud 账户发现的托管实例。',
      kindSshDesc: '通过 SSH 访问的 My King 安装。',
      labelTitle: '名称',
      ...
      headersDesc:
        '随发往此网关的每个 HTTP 和 WebSocket 请求一起发送——用于 Cloudflare Access 等访问代理（CF-Access-Client-Id / CF-Access-Client-Secret）。值加密存储。由 My King 管理的请求头（Authorization、Cookie、Host 等）会被忽略。',
      ...
      cloudAddHint: '提示：在上方登录 My King Cloud 可自动发现你的智能体——此表单仅用于手动注册已知的实例 URL。',
      save: '保存连接',
      saving: '保存中…',
      cancel: '取消',
      empty: '尚未注册任何连接。'
    ...
    gateway: {
      employeeEnrollment: {
        title: '连接 AI Work OS',
        description:
          '使用公司员工账号和密码登录。新电脑登录后会替换旧电脑的连接，My King 不\u2060会\u2060保\u2060存你的密\u2060码。',
        emailPlaceholder: '员工账号',
        ...
        reauthorize: '重新授权',
        unbind: '断开本机',
        unbindTitle: '从这台电脑退出 My King？',
        unbindDescription:
          'My King 只会删\u2060除这台电脑上的连接组件、密钥和后台任务，不\u2060会\u2060删\u2060除你的 SSH 配置或其他密钥。',
        diagnosticsTitle: 'My King 连接诊断',
        diagnosticsEmpty: '暂时没有连接组件诊断记录。',
        close: '关闭',
        stages: {
          idle: '等待员工登录',
          'validating-invitation': '正在验证员工账号',
          'confirming-identity': '正在确认员工身份',
          'configuring-secure-connection': '正在配置本机安全连接',
          'enabling-device-access': '正在启用员工电脑访问',
          'connecting-remote-gateway': '正在连接 My King 远程网关',
          'verifying-isolation': '正在验证设备隔离',
          connected: '已连接公司智能体',
          error: '连接需要处理'
        },
        completedStages: {
          'validating-invitation': '员工账号验证完成',
          'confirming-identity': '员工身份已确认',
          'configuring-secure-connection': '本机安全连接已配置',
          'enabling-device-access': '员工电脑访问已启用',
          'connecting-remote-gateway': 'My King 远程网关已连接',
          'verifying-isolation': '设备隔离已验证'
        },
        pendingStages: {
          'validating-invitation': '验证员工账号',
          'confirming-identity': '确认员工身份',
          'configuring-secure-connection': '配置本机安全连接',
          'enabling-device-access': '启用员工电脑访问',
          'connecting-remote-gateway': '连接 My King 远程网关',
          'verifying-isolation': '验证设备隔离'
        },
        errors: {
          'invite-expired': '邀请已失效，请联系管理员重新生成。',
          'invite-used': '邀请已经被其他电脑使用。',
          'permission-required': '当前电脑缺少系统权限，请点击“重新授权”。',
          'device-already-bound': '当前电脑已连接其他员工账号。请先断开本机，再使用你的账号登录。',
          'employee-mismatch': '员工身份不匹配，已停止连接。',
          'isolation-failed': '设备隔离验证失败，已停止连接。',
          'central-host-blocked': '这台电脑是公司总控机，安全策略禁止把它作为员工电脑接入。请在员工电脑上登录。',
          'ssh-service-failed': 'SSH 服务启动失败，请查看诊断信息。',
          ...
          'invalid-credentials': '员工账号或密码不正确。',
          'employee-agent-unavailable': '此员工账号尚未分配正在运行的 My King，请联系管理员。',
          'employee-agent-ambiguous': '此员工账号关联了多个正在运行的 My King，请联系管理员处理。',
          'not-ready': '公司尚未确认设备就绪，请稍后重试。',
          'enrollment-failed': '公司拒绝本次接入，请联系管理员。',
          'invalid-invitation': '本次员工账号连接请求无效，请重新登录。',
          'not-bound': '当前电脑尚未连接公司智能体。',
          'connector-failed': '安全连接启动失败，请重新连接。'
        }
      },
      loading: '正在加载网关设置...',
      ...
      remoteTitle: '远程网关',
      remoteDesc: '将此桌面外壳连接到远程 My King 后端。',
      remoteAuthHint: '托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      cloudTitle: 'My King Cloud',
      cloudDesc: '只需登录 My King Cloud 一次，即可从你账户下的智能体中选\u2060择——无需粘贴 URL。',
      cloudSignInTitle: 'My King Cloud',
      cloudSignIn: '登录 My King Cloud',
      cloudSignedIn: '已登录 My King Cloud',
      cloudNeedsSignIn: '登录 My King Cloud 以发现你账户下的智能体。',
      ...
      cloudDiscoverFailed: '无法加载你的 My King Cloud 智能体',
      cloudConnectFailed: '无法连接到该智能体',
      cloudSignInFailed: 'My King Cloud 登录失败',
      cloudSignedOutTitle: '已退出 My King Cloud',
      cloudSignedOutMessage: '已清除 My King Cloud 会话。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'My King Desktop 将使用已保存设置重新连接（界面保持打开）。',
      savedMessage: '已保存，下一次重启生效。',
      connectedTo: (baseUrl, version) => `已连接到 ${baseUrl}${version ? ` · My King ${version}` : ''}`,
      ...
      sshTitle: '通过 SSH 连接',
      sshDesc:
        'My King 会通过 SSH 在远程启动并以隧道连接到本应用——无需自行启动或暴露任何服务。前提：已具备到该主机的密钥 SSH 访问。',
      ...
      sshReachable: (host, platform) => `可连接：${host}（${platform}）——已找到 My King`,
    ...
    mcp: {
      ...
      deepLinkDescription:
        '一个链接请求将此 MCP 服务器添加到 My King。请检查下方的完整配置——它来自该链接，而非 My King。',
    ...
    providers: {
      ...
      intro: '使用订阅登录，无需复制 API 密钥。My King 会在应用中为你完成浏览器登录。',
      connected: '已连接',
      collapse: '收起',
      ...
      providerDescriptions: {
        nousPortal: '托管 My King 与 Nous 训练的模型',
        fireworks: '兼容 OpenAI 的直连模型 API',
        openrouter: '聚合数百个前沿模型',
        anthropic: 'Claude API 访问（Sonnet、Opus、Haiku）',
        xai: 'Grok 模型（SuperGrok / Premium+ 请使用 OAuth）',
        gemini: 'Google AI Studio（Gemini 1.5 / 2.0 / 2.5）',
        deepseek: '直连 DeepSeek API（V3.x、R1）',
        dashscope: '阿里云 DashScope — Qwen 及多厂商模型',
        glm: '智谱 GLM-4.6 与 Z.AI 托管端点',
        kimi: 'Moonshot Kimi K2 / 编程端点',
        kimiChina: 'Moonshot 中国端点',
        minimax: 'MiniMax-M2 与 Hailuo 国际端点',
        minimaxChina: 'MiniMax 中国大陆端点',
        huggingFace: 'Inference Providers — 通过 router.huggingface.co 提供 20+ 个开源模型',
        openCodeZen: '按量付费访问精选编程模型',
        openCodeGo: '$10/月订阅，使用开放编程模型',
        nvidia: 'build.nvidia.com 或你自己的本地 NIM 端点',
        ollama: '来自 ollama.com 的云端开源模型',
        lmStudio: '本地 LM Studio 服务器（兼容 OpenAI）',
        stepFun: 'StepFun Step Plan 编程模型',
        xiaomiMiMo: 'MiMo-V2.5 与 Xiaomi 专有模型',
        arceeAi: 'Arcee 托管的小型与中型模型',
        gmiCloud: 'GMI Cloud GPU 与模型服务',
        azureFoundry: 'Azure AI Foundry 自定义端点（兼容 OpenAI / Anthropic）',
        awsBedrock: '通过 AWS 配置文件和区域进行身份验证'
      },
      localEndpoint: {
        title: '本地 / 自定义端点',
        description: '将 My King 指向任意 OpenAI 兼容端点（Zyphra、vLLM、llama.cpp、Ollama 等）。'
      },
      customEndpoints: {
        title: '自定义端点',
        active: '正在使用',
        ...
        deleteEndpoint: '删除端点',
        emptyTitle: '暂无自定义端点',
        emptyDescription: '请在下方添加 OpenAI 兼容端点。',
        editTitle: '编辑端点',
        addTitle: '添加端点',
        name: '名称',
        providerId: '提供方 ID',
  ...
  starmap: {
    ...
    loading: '加载中…',
    emptyTitle: '尚无学习内容',
    emptyDesc: '当 My King 为你的工作构建技能和记忆时，会显示在这里。',
    share: '分享图谱',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '请重启 My King 以使用宠物功能——当前后端版本过旧。',
      empty: '没有匹配的宠物。',
      turnOff: '关闭',
      turnOn: '开启',
      installed: '已安装',
    ...
    generatePet: {
      ...
      staleBackend: '请更新 My King 以生成宠物。',
      backgroundHint: '你可以关闭此窗口——完成后 My King 会通知你。',
    ...
    nav: {
      ...
      settings: { title: '设置', detail: '配置 My King 桌面端' },
  ...
  messaging: {
    search: '搜索消息平台…',
    platformSummary: name => `通过 ${name} 与 My King 对话并管理连接。`,
    loading: '正在加载消息平台…',
    loadFailed: '消息平台加载失败',
    states: {
      connected: '已连接',
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接，然后把 My King 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 My King 自带的 WhatsApp 桥接，首次运行时扫描二维码，然后启用该平台。',
      bluebubbles:
        '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 My King 指向该 URL。',
  ...
  profiles: {
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 My King 环境：各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
    cloneFrom: '克隆来源',
    cloneFromNone: '无（空白）',
  ...
  cron: {
    ...
    newCron: '新建定时任务',
    emptyDescNew: '按 cron 表达式排程一个提示词。My King 会运行它，并把结果发送到你选择的目的地。',
  ...
  sidebar: {
    ...
    shiftClickHint: 'Shift+ 单击对话以置\u2060顶 · 拖动以重\u2060新\u2060排\u2060序',
    noWorkspace: '无工作区',
    projectEmpty: '暂无会话',
    noSessions: '暂无会话',
    ...
    projects: {
      ...
      createFailed: '无法创建项目',
      staleBackend: '请更新 My King 后端以创建项目——当前后端比桌面应用旧（设置 → 更新 → 后端）。',
      deleteConfirm: '这会从 My King 中移除已保存的项目。文件、git 仓库和工作树保持不变。',
      startWork: '新建工作树',
      ...
      worktreeStaleBackend: '请更新 My King 后端以在此远程连接上创建工作树 —— 该后端早于 git 工作树 API。',
  ...
  composer: {
    intro: {
      headline: '你的工作空间，一句话即可开始。',
      body: '搜索项目、编辑文件、运行测试、推进任务。告诉我目标，其余交给 My King。'
    },
    message: '消息',
    wakingProfile: profile => `正在唤醒 ${profile}…`,
    placeholderStarting: '正在启动 My King…',
    placeholderReconnecting: '正在重新连接 My King…',
    ...
    wakeWordListening: () => '唤醒词 — 正在监听',
    wakeWordOff: () => '唤醒词 — 已关闭',
    wakeWordPausedVoice: () => '唤醒词 — 语音对话期间暂停',
    lookupLoading: '查找中…',
    ...
    attachUrlDesc: 'My King 将抓取该页面并作为本回合的上下文。',
  ...
  updates: {
    ...
    applyingBody:
      'My King 更新器会在自己的窗口中接管，并在完成后自动重新打开 My King。更新期间请不要自行重新打开 My King。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 My King 会自动重新连接。',
    applyingClose: '此窗口会在更新期间关闭，随后 My King 会自动重新打开。',
    errorTitle: '更新未完成',
    errorBody: '没有数据丢失。你可以现在重试。',
    blockerTitle: '关闭本地预览以更新 My King？',
    blockerBody: 'My King 需要在更新前停止这些本地预览。这不会修改或删除你的文件。',
    foreignBlockerTitle: '关闭其他进程以更新 My King',
    foreignBlockerBody: 'My King 无法安全地自动关闭这些进程。请关闭拥有这些进程的应用、终端或服务，然后重试更新。',
    mixedBlockerBody: 'My King 可以关闭下方列出的本地预览。其他进程必须手动关闭，更新才能继续。',
  ...
  install: {
    ...
    setupChoiceTitle: '设置 My King Desktop',
    setupChoiceDesc: '将此应用连接到你已运行的 My King 网关，或在这台电脑上本地安装 My King。',
    connectExistingTitle: '连接到现有 My King',
    connectExistingShort: '连接现有环境',
    connectExistingDesc: '使用会话令牌或浏览器登录连接远程后端。不会启动本地安装。',
    installLocalTitle: '本地安装 My King',
    installLocalDesc: '下载 My King，创建 Python 环境，并在这台电脑上运行后端。',
    localStartUnavailable: '无法启动本地安装。请重启 My King Desktop 后重试。',
    remoteSetupTitle: '连接到现有 My King',
    ...
    backToSetup: '返回',
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 My King Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某个安装步骤失败。在 Windows 上，如果另一个 My King CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 My King 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    activeDesc: '这是一次性设置。My King 安装器正在下载依赖并配置你的机器。之后启动会跳过此步骤。',
  ...
  onboarding: {
    headerTitle: '开始设置 My King Agent',
    headerDesc: '连接模型提供方即可开始对话。大多数选项只需一次点击。',
    preparingInstall: 'My King 正在完成安装。首次运行通常不到一分钟。',
    starting: '正在启动 My King…',
    ...
    recommended: '推荐',
    connected: '已连接',
    featuredPitch: '一个订阅，300+ 前沿模型 — 运行 My King 的推荐方式',
    ...
    flowSubtitles: {
      ...
      device_code: '在浏览器中打开验证页面 — My King 会自动连接',
    ...
    authorizeThere: '请在那里授权 My King。',
    copyAuthCode: '复制授权码并粘贴到下面。',
    pasteAuthCode: '粘贴授权码',
    reopenAuthPage: '重新打开授权页面',
    autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权 My King，连接会自动完成，无需复制或粘贴。`,
  ...
  preview: {
    ...
    largeTitle: '此文件较大',
    largeBody: (label, size) => `${label} 大小为 ${size}。My King 只会显示前 512 KB。`,
    previewAnyway: '仍然预览',
    ...
    web: {
      ...
      failedToLoad: '预览加载失败',
      tryAgain: '重试',
      restarting: 'My King 正在重启...',
      askRestart: '让 My King 重启服务器',
      lookingRestart: taskId => `My King 正在查找要重启的预览服务器 (${taskId})`,
      restartingTitle: '正在重启预览服务器',
      restartingMessage: 'My King 正在后台工作。可在预览控制台查看进度。',
      ...
      blankPageBody: '在上方输入地址开始浏览，或让 My King 打开一个页面。',
      finishedRestarting: message => `My King 已完成预览服务器重启${message ? `: ${message}` : ''}`,
      failedRestarting: message => `服务器重启失败：${message}`,
      unknownError: '未知错误',
      restartedTitle: '预览服务器已重启',
      reloadingNow: '正在重新加载预览。',
      restartFailedTitle: '预览重启失败',
      restartFailedMessage: 'My King 无法重启服务器。',
      stillWorking: 'My King 仍在工作，但还没有收到重启结果。服务器命令可能正在前台运行。',
  ...
  assistant: {
    thread: {
      ...
      showEarlier: '显示更早的消息',
      loadingResponse: 'My King 正在加载回复',
    ...
    approval: {
      gatewayDisconnected: 'My King 网关未连接',
      sendFailed: '无法发送审批响应',
      run: '运行',
      command: '命令',
      moreOptions: '更多审批选项',
      ...
      alwaysDescription: pattern =>
        `这会将“${pattern}”模式加入永久允许列表 (~/.myking/config.yaml)。My King 对类似命令将不再询问，包括当前会话和未来会话。`,
      alwaysAllow: '始终允许'
    },
    clarify: {
      notReady: '澄清请求尚未就绪',
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: 'My King 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
    sudoPlaceholder: 'sudo 密码',
    secretTitle: '需要密钥',
    secretDesc: 'My King 需要一个凭据才能继续。',
  ...
  desktop: {
    ...
    fileStillSaving: '文件仍在写入，请等待保存完成后重试。',
    ...
    restartToUseSaveImage: '重启 My King 桌面版后可使用保存图片。',
    restartToSaveImages: '重启 My King 桌面版以保存图片',
