...
export const ja = defineLocale({
  ...
  boot: {
    ready: 'My King Desktop の準備ができました',
    desktopBootFailedWithMessage: message => `デスクトップの起動に失敗しました: ${message}`,
    steps: {
      connectingGateway: 'ライブデスクトップゲートウェイに接続中',
      loadingSettings: 'My King の設定を読み込み中',
      loadingSessions: '最近のセッションを読み込み中',
      retryingRemoteBackend: 'リモート My King バックエンドに再接続中…',
      startingDesktopConnection: 'デスクトップ接続を開始中',
      startingHermesDesktop: 'My King Desktop を起動中…'
    },
    errors: {
      backgroundExited: 'My King バックグラウンドプロセスが終了しました。',
      backgroundExitedDuringStartup: '起動中に My King バックグラウンドプロセスが終了しました。',
      backendStopped: 'バックエンドが停止しました',
      desktopBootFailed: 'デスクトップの起動に失敗しました',
      employeeEnrollmentRequired: '会社の従業員アカウントでサインインして接続してください。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'My King バックエンドがこのデスクトップビルドより古く、正常に動作しない場合があります。更新して揃えてください。',
    installMethodUnsupportedTitle: 'サポート対象外のインストール方法',
    updateHermes: 'My King を更新',
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。My King Desktop を再起動してください。',
    ...
    native: {
      ...
      rejectAction: '拒否',
      inputTitle: '入力が必要です',
      inputBody: 'My King が応答を待っています。',
      turnDoneTitle: 'My King が完了しました',
      turnDoneBody: '',
  ...
  settings: {
    ...
    resetConfirm: 'すべての設定を My King のデフォルトに戻しますか？',
    ...
    notifications: {
      ...
      focusedHint: '完了通知は My King がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
        ...
        input: {
          label: '入力が必要',
          description: 'My King が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
          description: 'My King がバックグラウンドのときにターンが完了しました。'
        },
        turnError: {
          label: 'ターン失敗',
        ...
        plugin: {
          label: 'プラグイン通知',
          description: 'My King がバックグラウンドの間に、デスクトッププラグインが通知を送信しました。'
        }
      },
      test: 'テスト通知を送信',
      testTitle: 'My King',
    ...
    appearance: {
      ...
      colorMode: 'カラーモード',
      colorModeDesc: '固定モードを選ぶか、My King をシステム設定に合わせます。',
      ...
      backdropDesc: '会話の背後に表示される控えめな My King ガラスオーラ。',
      introSplashTitle: 'イントロ表示',
      introSplashDesc: '空のチャットに表示されるワードマークとプロンプト。',
      reactionsTitle: 'メッセージリアクション',
      reactionsDesc:
        'iMessage風の絵文字タップバック — メッセージにリアクションでき、My Kingもあなたのメッセージにリアクションします。',
      ...
      themeNoMatches: query => `「${query}」に一致するインストール済みテーマはありません。`,
      themeNames: { nous: 'クラシック', 'liquid-glass': 'リキッドガラス' },
      themeDescriptions: {
        nous: '落ち着いたニュートラルカラーと My King ブルーのアクセント。',
        'liquid-glass': 'パール、シルバー、Apple ブルーを重ねた光学ガラス。'
      },
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、My King の状態に反応します。',
        restartHint:
          'ペット機能には再起動が必要です。この機能が追加される前に起動したアプリが動作中です。My King を終了して再度開き、このページに戻ってください。',
        scaleTitle: 'サイズ',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'My King がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'My King が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'My King が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを My King がどう扱うかを設定します。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'アプリから My King 自身を更新するとき、ローカルのソース変更を保持するか破棄するかを選びます。ターミナル更新では常に確認されます。'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'My King バックエンドは現在のデスクトップパッケージより新しい状態です。バックエンド更新がブランド UI を置き換えることはありません。新しい UI 機能が必要な場合は、次の検証済み My King デスクトップパッケージをインストールしてください。',
      bundleOutOfSyncAction: 'My King の更新を開く',
      updates: '更新',
      checkNow: '今すぐ確認',
      checking: '確認中…',
      seeWhatsNew: '新機能を見る',
      ...
      automaticUpdatesDesc:
        'My King はバックエンドを個別に更新します。検証済みの My King デスクトップパッケージを導入するまで、ブランド UI とレイアウトは変更されません。',
      ...
      uninstall: {
        dangerZone: '危険な操作',
        checking: 'インストール内容を確認中…',
        confirmTitle: 'アンインストールの確認',
        confirmDescription: consequence => `${consequence}を削除します。この操作は取り消せません。`,
        appPath: path => `アプリ：${path}`,
        failedStart: 'アンインストールを開始できませんでした。',
        uninstalling: 'アンインストール中…',
        confirmAction: 'アンインストールする',
        cancel: 'キャンセル',
        title: 'My King をアンインストール',
        description: '削除する範囲を選択してください。完了のためアプリが終了します。後で再インストールできます。',
        options: {
          gui: {
            title: 'デスクトップアプリのみ削除',
            description: 'My King エージェント、設定、チャットは残します。',
            consequence: 'デスクトップアプリとローカルアプリデータ'
          },
          lite: {
            title: 'アプリとエージェントを削除し、データは保持',
            description: '設定、チャット、シークレットを再インストール用に残します。',
            consequence: 'デスクトップアプリと My King エージェント（設定、チャット、シークレットは保持）'
          },
          full: {
            title: 'すべて削除',
            description: 'アプリ、エージェント、チャット、ジョブ、シークレット、ログを削除します。',
            consequence: 'デスクトップアプリ、My King エージェント、設定、チャット、シークレット、ログ'
          }
        }
      }
    },
    ...
    quickEntry: {
      enabledTitle: 'クイック入力',
      enabledDesc:
        'グローバルショートカットで小さな入力欄をどこからでも呼び出し、My King を開かずにプロンプトを送信します。',
    ...
    gateway: {
      employeeEnrollment: {
        ...
        description:
          '会社の従業員アカウントとパスワードでサインインしてください。新しいコンピューターでサインインすると以前の接続は置き換えられ、My King はパスワードを保存しません。',
        emailPlaceholder: '従業員アカウント',
        ...
        reauthorize: '再認証',
        unbind: 'このコンピュータを切断',
        unbindTitle: 'このコンピュータで My King からサインアウトしますか？',
        unbindDescription:
          'このコンピュータ上の My King 接続コンポーネント、鍵、バックグラウンドタスクだけを削除します。',
        diagnosticsTitle: 'My King Employee Connector の診断',
        diagnosticsEmpty: '接続コンポーネントの診断記録はまだありません。',
        close: '閉じる',
        stages: {
          idle: '従業員のサインインを待機中',
          'validating-invitation': '従業員アカウントを確認中',
          'confirming-identity': '従業員の本人確認中',
          'configuring-secure-connection': 'このデバイスの安全な接続を設定中',
          'enabling-device-access': '従業員デバイスへのアクセスを有効化中',
          'connecting-remote-gateway': 'My King リモートゲートウェイに接続中',
          'verifying-isolation': 'デバイス分離を検証中',
          connected: '会社のエージェントに接続しました',
          error: '接続を確認してください'
        },
        completedStages: {
          'validating-invitation': '従業員アカウントを確認しました',
          'confirming-identity': '従業員の本人確認が完了しました',
          'configuring-secure-connection': 'このデバイスの安全な接続を設定しました',
          'enabling-device-access': '従業員デバイスへのアクセスを有効にしました',
          'connecting-remote-gateway': 'My King リモートゲートウェイに接続しました',
          'verifying-isolation': 'デバイス分離を検証しました'
        },
        pendingStages: {
          'validating-invitation': '従業員アカウントを確認',
          'confirming-identity': '従業員の本人確認',
          'configuring-secure-connection': 'このデバイスの安全な接続を設定',
          'enabling-device-access': '従業員デバイスへのアクセスを有効化',
          'connecting-remote-gateway': 'My King リモートゲートウェイに接続',
          'verifying-isolation': 'デバイス分離を検証'
        },
        errors: {
          'invite-expired': '招待の有効期限が切れました。管理者に再発行を依頼してください。',
          'invite-used': 'この招待は別のデバイスで使用済みです。',
          'permission-required': 'システム権限が必要です。「再承認」をクリックしてください。',
          'device-already-bound':
            'このコンピュータは別の従業員アカウントに接続されています。切断してからサインインしてください。',
          'employee-mismatch': '従業員 ID が一致しないため、接続を停止しました。',
          'isolation-failed': 'デバイス分離の確認に失敗したため、接続を停止しました。',
          'central-host-blocked':
            'このコンピューターは会社の管理ホストのため、従業員デバイスとして使用できません。従業員のコンピューターでサインインしてください。',
          'ssh-service-failed': 'SSH サービスを開始できません。診断情報を確認してください。',
          ...
          'invalid-credentials': '従業員アカウントまたはパスワードが正しくありません。',
          'employee-agent-unavailable':
            'この従業員アカウントには実行中の My King が割り当てられていません。管理者に連絡してください。',
          'employee-agent-ambiguous':
            'この従業員アカウントには複数の実行中 My King が割り当てられています。管理者に連絡してください。',
          'not-ready': '会社側でこのデバイスの準備完了が確認されていません。しばらくしてから再試行してください。',
          'enrollment-failed': '会社がこの登録を拒否しました。管理者に連絡してください。',
          'invalid-invitation': '従業員アカウントの接続要求が無効です。もう一度サインインしてください。',
          'not-bound': 'このデバイスはまだ接続されていません。',
          'connector-failed': '安全な接続を開始できません。もう一度お試しください。'
        }
      },
      ...
      restartingMessage: 'My King Desktop は保存された設定を使用して再接続します。',
      savedMessage: '次回起動時に保存されます。',
      connectedTo: (baseUrl, version) => `${baseUrl}${version ? ` · My King ${version}` : ''} に接続しました`,
      ...
      sshTitle: 'SSH で接続',
      sshDesc:
        'My King は SSH 経由でリモート上に起動され、このアプリにトンネルされます。リモート側で何かを起動・公開する必要はありません。ホストへの鍵ベースの SSH アクセスが前提です。',
      ...
      sshReachable: (host, platform) => `接続可能: ${host}（${platform}）— My King を検出`,
      sshIncompleteHost: '接続する前に SSH ホストを入力してください。',
      sshErrUnreachable: 'SSH でそのホストに到達できませんでした。ホスト、ポート、ネットワークを確認してください。',
      sshErrAuth:
        'SSH 認証に失敗しました。鍵を ssh-agent に読み込む（ssh-add）か、~/.ssh/config に IdentityFile を設定してください。My King は非対話的に ssh を実行します。',
      sshErrHostKey:
      ...
      sshErrUpdateRequired: 'Desktop SSH で接続する前に、リモートホストの My King を更新してください。',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'リンクがこの MCP サーバーを My King に追加するよう要求しました。下の設定はリンク側から来たものです。内容を必ず確認してください。',
    ...
    providers: {
      ...
      intro:
        'サブスクリプションでサインインします。API キーのコピーは不要です。My King がアプリ内でブラウザーサインインを代行します。',
      connected: '接続済み',
      collapse: '折りたたむ',
      ...
      providerDescriptions: {
        nousPortal: 'My King と Nous がトレーニングしたモデルをホスト',
        fireworks: 'OpenAI 互換の直接モデル API',
        openrouter: '数百の最先端モデルを集約',
        anthropic: 'Claude API へのアクセス（Sonnet、Opus、Haiku）',
        xai: 'Grok モデル（SuperGrok / Premium+ は OAuth を使用）',
        gemini: 'Google AI Studio（Gemini 1.5 / 2.0 / 2.5）',
        deepseek: 'DeepSeek API への直接アクセス（V3.x、R1）',
        dashscope: 'Alibaba Cloud DashScope — Qwen と複数ベンダーのモデル',
        glm: 'Zhipu GLM-4.6 と Z.AI のホスト型エンドポイント',
        kimi: 'Moonshot Kimi K2 / コーディングエンドポイント',
        kimiChina: 'Moonshot 中国エンドポイント',
        minimax: 'MiniMax-M2 と Hailuo の国際エンドポイント',
        minimaxChina: 'MiniMax 中国本土エンドポイント',
        huggingFace: 'Inference Providers — router.huggingface.co 経由で 20 以上のオープンモデル',
        openCodeZen: '厳選されたコーディングモデルを従量課金で利用',
        openCodeGo: '$10/月のサブスクリプションでオープンコーディングモデルを利用',
        nvidia: 'build.nvidia.com または独自のローカル NIM エンドポイント',
        ollama: 'ollama.com のクラウドホスト型オープンモデル',
        lmStudio: 'ローカル LM Studio サーバー（OpenAI 互換）',
        stepFun: 'StepFun Step Plan コーディングモデル',
        xiaomiMiMo: 'MiMo-V2.5 と Xiaomi 独自モデル',
        arceeAi: 'Arcee がホストする小型・中型モデル',
        gmiCloud: 'GMI Cloud GPU + モデルサービング',
        azureFoundry: 'Azure AI Foundry カスタムエンドポイント（OpenAI / Anthropic 互換）',
        awsBedrock: 'AWS プロファイルとリージョンで認証'
      },
      localEndpoint: {
      ...
      customEndpoints: {
        ...
        emptyDescription: '下で OpenAI 互換エンドポイントを追加してください。',
        ...
        auto: '自動',
        apiKey: 'API キー',
        keepCurrentKey: '現在のキーを保持する場合は空欄',
        optional: '任意',
        ...
        saveFailed: '保存に失敗しました',
        reachableWithModels: count => `エンドポイントに接続できました。${count} 個のモデルが見つかりました。`,
  ...
  starmap: {
    ...
    emptyDesc: 'My King がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
    close: 'エージェントを閉じる',
    title: 'スポーンツリー',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'ペット機能を使うには My King を再起動してください。',
      empty: '一致するペットがありません。',
      turnOff: 'オフ',
      turnOn: 'オン',
      installed: 'インストール済み',
    ...
    generatePet: {
      ...
      staleBackend: 'ペットを生成するには My King を更新してください。',
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら My King が通知します。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: 'My King デスクトップを設定' },
  ...
  messaging: {
    search: 'メッセージングを検索...',
    platformSummary: name => `${name} から My King を利用します。`,
  ...
  profiles: {
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した My King 環境です：設定、スキル、SOUL.md が別々になります。',
    nameLabel: '名前',
    cloneFrom: '複製元',
    cloneFromNone: 'なし（空）',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      saveFailed: 'My King はモデルの変更を保存しませんでした。'
    },
    search: 'Cron ジョブを検索...',
    ...
    newCron: '新しい Cron',
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。My King が実行して、選択した宛先に結果を送信します。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'プロジェクトを作成するには My King バックエンドを更新してください。バックエンドがこのデスクトップアプリより古いです（設定 → 更新 → バックエンド）。',
      deleteConfirm:
        'My King から保存済みプロジェクトを削除します。ファイル・git リポジトリ・ワークツリーはそのまま残ります。',
      startWork: '新しいワークツリー',
      ...
      worktreeStaleBackend:
        'このリモート接続でワークツリーを作成するには My King バックエンドを更新してください — git ワークツリー API 以前のバージョンです。',
  ...
  composer: {
    intro: {
      headline: 'あなたのワークスペースを、一言で始めよう。',
      body: 'リポジトリの検索、ファイル編集、テスト実行、タスクの前進まで。目標を伝えれば、あとは My King が進めます。'
    },
    message: 'メッセージ',
    wakingProfile: profile => `${profile} を起動中…`,
    placeholderStarting: 'My King を起動中...',
    placeholderReconnecting: 'My King に再接続中…',
    ...
    wakeWordListening: () => 'ウェイクワード — 待機中',
    wakeWordOff: () => 'ウェイクワード — オフ',
    wakeWordPausedVoice: () => 'ウェイクワード — 音声チャット中は一時停止',
    lookupLoading: '検索中…',
    ...
    attachUrlDesc: 'My King がページを取得し、このターンのコンテキストとして含めます。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'My King にコミットと PR を任せる',
  ...
  updates: {
    ...
    unsupportedMessage: 'このバージョンの My King はアプリ内から自分を更新できません。',
    ...
    availableBody: '新しいバージョンの My King をインストールする準備ができています。',
    availableTitleBackend: 'バックエンドの更新があります',
    availableBodyBackend: '接続中の My King バックエンドの新しいバージョンをインストールできます。',
    ...
    applyingBody:
      'My King アップデーターが独自のウィンドウで引き継ぎ、完了後に自動的に My King を再度開きます。更新中はご自分で My King を開き直さないでください。',
    applyingBodyBackend:
      'リモートバックエンドが更新を適用して再起動します。復帰すると My King が自動的に再接続します。',
    applyingClose: 'このウィンドウは更新中に閉じ、その後 My King が自動的に再度開きます。',
    errorTitle: '更新が完了しませんでした',
    errorBody: 'ご安心ください。何も失われていません。今すぐ再試行できます。',
    blockerTitle: 'My King を更新するためにローカルプレビューを閉じますか？',
    blockerBody:
      '更新する前に、これらのローカルプレビューを停止する必要があります。ファイルが変更または削除されることはありません。',
    foreignBlockerTitle: '他のプロセスを閉じて My King を更新',
    foreignBlockerBody:
      'My King はこれらのプロセスを安全に自動終了できません。各プロセスを所有するアプリ、ターミナル、またはサービスを閉じてから、もう一度更新してください。',
    mixedBlockerBody:
      'My King は以下のローカルプレビューを閉じることができます。更新を続けるには、他のプロセスを手動で閉じる必要があります。',
  ...
  install: {
    ...
    oneTimeTitle: 'My King には一度限りのインストールが必要です',
    ...
    setupChoiceTitle: 'My King Desktop をセットアップ',
    setupChoiceDesc:
      'すでに実行している My King ゲートウェイに接続するか、このコンピューターに My King をローカルインストールします。',
    connectExistingTitle: '既存の My King に接続',
    connectExistingShort: '既存環境に接続',
    connectExistingDesc:
      'セッショントークンまたはブラウザーサインインでリモートバックエンドを使用します。ローカルインストールは開始されません。',
    installLocalTitle: 'My King をローカルにインストール',
    installLocalDesc: 'My King をダウンロードし、Python 環境を作成して、このコンピューターでバックエンドを実行します。',
    localStartUnavailable:
      'ローカルインストールを開始できません。My King Desktop を再起動して、もう一度お試しください。',
    remoteSetupTitle: '既存の My King に接続',
    remoteSetupDesc:
    ...
    settingUpTitle: 'My King Agent を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の My King CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の My King インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
      'これは一回限りのセットアップです。My King インストーラーが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  onboarding: {
    headerTitle: 'My King Agent のセットアップをしましょう',
    headerDesc: 'チャットを始めるにはモデルプロバイダーを接続してください。ほとんどのオプションはワンクリックです。',
    preparingInstall: 'My King はインストールを完了中です。初回実行では通常 1 分以内に完了します。',
    starting: 'My King を起動中…',
    ...
    recommended: '推奨',
    connected: '接続済み',
    featuredPitch: '1 つのサブスクリプションで 300 以上の最先端モデル — My King を実行するための推奨方法',
    ...
    flowSubtitles: {
      ...
      device_code: 'ブラウザーで確認ページを開きます — My King が自動接続します',
    ...
    authorizeThere: 'そこで My King を承認してください。',
    copyAuthCode: '認証コードをコピーして以下に貼り付けてください。',
    pasteAuthCode: '認証コードを貼り付け',
    reopenAuthPage: '認証ページを再度開く',
    autoBrowser: provider =>
      `${provider} をブラウザーで開きました。My King をそこで承認すれば自動接続されます。コピーや貼り付けは不要です。`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} は ${size} です。My King は最初の 512 KB のみを表示します。`,
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'My King を再起動中...',
      askRestart: 'My King にサーバーの再起動を依頼',
      lookingRestart: taskId => `My King は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'My King はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      blankPageBody: '上のアドレス欄に入力するか、My King にページを開くよう頼んでください。',
      finishedRestarting: message =>
        `My King がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'My King がサーバーを再起動できませんでした。',
      stillWorking:
        'My King はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'My King が応答を読み込み中',
    ...
    approval: {
      gatewayDisconnected: 'My King ゲートウェイが接続されていません',
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.myking/config.yaml) に追加されます。My King はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
  ...
  prompts: {
    ...
    sudoTitle: '管理者パスワード',
    sudoDesc:
      'My King は特権コマンドを実行するために sudo パスワードが必要です。ローカルエージェントにのみ送信されます。',
    sudoPlaceholder: 'sudo パスワード',
    secretTitle: 'シークレットが必要です',
    secretDesc: 'My King は続行するための認証情報が必要です。',
  ...
  desktop: {
    ...
    fileStillSaving: 'ファイルはまだ書き込み中です。保存が完了してから再試行してください。',
    ...
    restartToUseSaveImage: '画像を保存するには My King Desktop を再起動してください。',
    restartToSaveImages: '画像を保存するには My King Desktop を再起動してください',
