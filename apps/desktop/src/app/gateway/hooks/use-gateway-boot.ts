...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting My King...". Once the
    ...
    const attemptReconnect = async (propagateReauth = false) => {
      ...
      try {
        ...
        // "Starting My King…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to My King gateway". resolveGatewayWsUrl
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current
            .refreshHermesConfig()
            .catch(err => console.warn('Failed to refresh My King config after gateway connect', err)),
          ...
          // retry it), not brick boot behind the "My King couldn't start"
