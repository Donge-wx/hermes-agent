...
export function UninstallSection() {
  ...
  const options: ModeOption[] = OPTION_META.map(option => ({ ...option, ...u.options[option.mode] }))
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (!result.ok) {
        setError(result.message || result.error || u.failedStart)
        setRunning(false)
        setPending(null)
      }
      ...
      <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3">
        {loading ? (
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">{u.confirmDescription(pendingOption.consequence)}</p>
