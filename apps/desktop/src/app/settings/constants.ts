...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  agent: {
    ...
    maxTurns: 'Upper bound for tool-calling turns before My King stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters My King can read from one file request.',
  approvals: {
    mode: 'How My King handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When My King updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
