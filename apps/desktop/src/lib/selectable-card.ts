...
export function selectableCardClass({ active, prominent }: SelectableCardState): string {
  return cn(
    'selectable-card rounded-lg border transition-colors',
    active
      ? 'selectable-card-active border-primary bg-primary/[0.06] ring-2 ring-primary/20'
      : prominent
        ? 'selectable-card-prominent border-(--ui-stroke-tertiary) bg-(--ui-bg-quinary) hover:bg-(--chrome-action-hover)'
        : 'selectable-card-muted border-transparent bg-transparent text-(--ui-text-tertiary) hover:border-(--ui-stroke-tertiary) hover:bg-(--ui-bg-quinary)'
  )
}
