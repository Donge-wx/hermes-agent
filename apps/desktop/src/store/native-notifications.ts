...
// "Backgrounded" = the user isn't on My King. `document.hidden` only flips when
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `myking://index-network/intent/1`.
   */
  activate?: string
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `myking://index-network/intent/1`). */
...
export interface PluginNativeNotificationInput {
   ...
   * (`myking://index-network/intent/1`), a hash path (`/index-network/intent/1`),
 ...
 *  user is away from My King — the in-app toast (`host.notify`) covers the
 *  foreground case. */
