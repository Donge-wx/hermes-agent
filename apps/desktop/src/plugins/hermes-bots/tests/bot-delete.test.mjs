...
test('regression: the bot context menu exposes a destructive delete action and confirmation', () => {
  assert.match(pluginSource, /ContextMenuItem, \{[\s\S]*?variant: 'destructive'[\s\S]*?children: t\('actions\.delete'\)/)
  assert.match(pluginSource, /isDefaultBot\(bot\) \? null : jsx\(ContextMenuSeparator/)
  assert.match(pluginSource, /ConfirmDialog/)
  assert.match(pluginSource, /title: t\('agent\.deleteTitle'\)/)
  assert.match(pluginSource, /t\('agent\.deletePrefix'\)[\s\S]*?t\('agent\.deletePath'\)[\s\S]*?t\('agent\.deleteSuffix'\)/)
})
