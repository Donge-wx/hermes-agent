...
describe('assistant-ui streaming renderer', () => {
  ...
  it('renders assistant text incrementally before completion', async () => {
    ...
    expect(screen.getByRole('status', { name: 'My King is loading a response' })).toBeTruthy()

    await waitFor(() => {
      expect(container.textContent).toContain('first chunk')
    })
    expect(container.textContent).not.toContain('second chunk')
    expect(screen.queryByRole('status', { name: 'My King is loading a response' })).toBeNull()
  ...
  it('marks live reasoning as an active presentation state and exposes its open state', () => {
    const { container } = render(<RunningReasoningHarness />)
    const disclosure = container.querySelector('[data-slot="aui_thinking-disclosure"]')

    expect(disclosure?.getAttribute('data-state')).toBe('active')
