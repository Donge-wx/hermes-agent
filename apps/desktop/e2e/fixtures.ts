...
async function closeElectronApp(app: ElectronApplication, timeoutMs = 5_000): Promise<void> {
  ...
  const isAlive = (pid: number) => {
    try {
      ...
      return Boolean(state && !state.startsWith('Z') && !state.includes('E'))
    } catch {
      return false
    }
  }
  ...
  while (targets.some(isAlive)) {
    if (Date.now() >= deadline) {
      ...
      const survivors = targets
        .filter(isAlive)
        .map(pid =>
          spawnSync('ps', ['-o', 'pid=,ppid=,state=,command=', '-p', String(pid)], { encoding: 'utf8' }).stdout.trim()
        )
        .filter(Boolean)

      if (survivors.length) {
        throw new Error(
          `E2E Electron process did not exit after bounded SIGTERM/SIGKILL cleanup:\n${survivors.join('\n')}`
        )
      }
