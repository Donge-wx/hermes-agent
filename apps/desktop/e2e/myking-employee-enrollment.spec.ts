...
test('renders every My King employee enrollment state in the real Electron window', async ({ browserName: _browserName }, testInfo) => {
  ...
  const resize = async (width: number, height = 800) => {
    ...
    await expect.poll(() => fixture.page.evaluate(() => [window.innerWidth, window.innerHeight])).toEqual([width, height])
  }
  ...
  try {
    ...
    await expect(fixture.page.getByText('连接公司智能体', { exact: true })).toBeVisible()
