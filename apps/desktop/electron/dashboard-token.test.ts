...
test('fetchPublicText rejects unsupported protocols', async () => {
  await assert.rejects(() => fetchPublicText('file:///tmp/index.html'), /Unsupported My King backend URL protocol/)
})
...
test('adoptServedDashboardToken falls back to the spawn token when the fetch fails', async () => {
  ...
  assert.equal(token, 'spawn-token')
  assert.equal(logs.length, 1)
  assert.match(logs[0], /could not read served dashboard token \(My King backend\): boom/)
})
