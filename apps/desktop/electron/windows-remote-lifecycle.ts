...
async function probeWindowsRemote(ssh, explicitHermesPath = '') {
  ...
  const script = [
    ...
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote My King Python runtime was not found."}',
...
async function connectWindowsRemote(deps) {
  ...
  if (!inspection.supported) {
    const error: any = new Error('Update My King on the remote Windows host before connecting with Desktop SSH.')
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="My King SSH"', 'powershell.exe -NoLogo')
