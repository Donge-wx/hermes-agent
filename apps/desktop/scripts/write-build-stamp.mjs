...
function fromCI() {
  // A branded desktop can be built from a private UI branch while still
  // bootstrapping the public, audited Hermes backend commit. GitHub's own
  // GITHUB_SHA points at the private UI commit and cannot be overridden, so
  // release CI may provide an explicit backend pin here.
  const sha = process.env.HERMES_BUILD_STAMP_SHA || process.env.GITHUB_SHA
  if (!sha) return null
  const branch =
    process.env.HERMES_BUILD_STAMP_BRANCH || process.env.GITHUB_REF_NAME || process.env.GITHUB_HEAD_REF || null
  return {
    commit: sha,
    branch: branch,
